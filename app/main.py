...
def create_app() -> FastAPI:
    ...
    async def maintenance_and_security_headers(request: Request, call_next):
        ...
        if not path.startswith("/health") and not path.startswith("/uploads"):
            try:
                db = next(getattr(request.app.state, "_db_session_factory", lambda: iter(()))())  # pragma: no cover
            except Exception:
                db = None
            if db is None:
            ...
            try:
                if SiteSettingService(db).is_maintenance_mode():
                    ...
                    allowed = path in {"/admin/login", "/admin/logout"} or path.startswith("/health")
    ...
    async def unhandled_exception_handler(request: Request, exc: Exception):
        ...
        if request.url.path.startswith("/admin"):
            return HTMLResponse("<h1>Internal Server Error</h1><p>Ошибка уже записана в лог. Обнови страницу позже или проверь traceback.</p>", status_code=500)
