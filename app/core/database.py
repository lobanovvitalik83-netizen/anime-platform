...
def _ensure_runtime_schema() -> None:
    ...
    if 'report_tickets' in inspector.get_table_names():
        existing_columns = {column['name'] for column in inspector.get_columns('report_tickets')}
        statements = []
        if 'source_platform' not in existing_columns:
            statements.append("ALTER TABLE report_tickets ADD COLUMN source_platform VARCHAR(32) NOT NULL DEFAULT 'telegram'")
        if 'vk_user_id' not in existing_columns:
            statements.append('ALTER TABLE report_tickets ADD COLUMN vk_user_id BIGINT')
        if 'vk_peer_id' not in existing_columns:
            statements.append('ALTER TABLE report_tickets ADD COLUMN vk_peer_id BIGINT')
        if 'vk_full_name' not in existing_columns:
            statements.append('ALTER TABLE report_tickets ADD COLUMN vk_full_name VARCHAR(255)')
        with engine.begin() as connection:
            for statement in statements:
                connection.execute(text(statement))
    ...
    if 'report_messages' in inspector.get_table_names():
        ...
        statements = []
        if 'vk_user_id' not in existing_columns:
            statements.append('ALTER TABLE report_messages ADD COLUMN vk_user_id BIGINT')
        with engine.begin() as connection:
            for statement in statements:
                connection.execute(text(statement))
            if dialect_name.startswith('postgres') and 'tg_user_id' in existing_columns:
