...
class ImportExportService:
    ...
    def export_titles_csv(self) -> str:
        ...
        for item in self.media.list_titles():
            rows.append({"id": item.id, "type": item.type, "title": item.title, "original_title": item.original_title or "", "description": item.description or "", "year": item.year or "", "status": item.status, "created_at": item.created_at.isoformat() if item.created_at else ""})
        return self._render_csv(rows)

    def export_seasons_csv(self) -> str:
        rows = []
        for item in self.media.list_seasons():
            rows.append({"id": item.id, "title_id": item.title_id, "season_number": item.season_number, "name": item.name or "", "description": item.description or "", "created_at": item.created_at.isoformat() if item.created_at else ""})
        return self._render_csv(rows)

    def export_episodes_csv(self) -> str:
        rows = []
        for item in self.media.list_episodes():
            rows.append({"id": item.id, "title_id": item.title_id, "season_id": item.season_id or "", "episode_number": item.episode_number, "name": item.name or "", "synopsis": item.synopsis or "", "status": item.status, "created_at": item.created_at.isoformat() if item.created_at else ""})
        return self._render_csv(rows)

    def export_assets_csv(self) -> str:
        rows = []
        for item in self.assets.list_assets():
            rows.append({"id": item.id, "title_id": item.title_id or "", "season_id": item.season_id or "", "episode_id": item.episode_id or "", "asset_type": item.asset_type, "storage_kind": item.storage_kind, "storage_provider": getattr(item, "storage_provider", "") or "", "storage_object_key": getattr(item, "storage_object_key", "") or "", "external_url": item.external_url or "", "mime_type": item.mime_type or "", "is_primary": item.is_primary, "created_at": item.created_at.isoformat() if item.created_at else ""})
        return self._render_csv(rows)
    ...
    def export_reports_csv(self) -> str:
        ...
        for item in self.reports.list_tickets():
            rows.append({"id": item.id, "status": item.status, "tg_user_id": item.tg_user_id, "tg_username": item.tg_username or "", "tg_full_name": item.tg_full_name or "", "assigned_admin_id": item.assigned_admin_id or "", "last_message_preview": item.last_message_preview or "", "updated_at": item.updated_at.isoformat() if item.updated_at else ""})
        return self._render_csv(rows)
    ...
    def template_titles_csv(self) -> str:
        return "type,title,original_title,description,year,status\nanime,Naruto,ナルト,Test description,2002,draft\n"
    ...
    def import_titles_csv(self, admin_id: int, filename: str, content: bytes):
        ...
        for index, row in enumerate(reader, start=2):
            try:
                self.media.create_title({"type": (row.get("type") or "").strip(), "title": (row.get("title") or "").strip(), "original_title": (row.get("original_title") or "").strip() or None, "description": (row.get("description") or "").strip() or None, "year": int(row["year"]) if (row.get("year") or "").strip() else None, "status": (row.get("status") or "draft").strip()})
                success_rows += 1
