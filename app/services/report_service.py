...
class ReportService:
    ...
    def create_or_append_from_telegram(self, *, tg_user_id: int, tg_chat_id: int, tg_username: str | None, tg_full_name: str | None, body: str):
        ...
        if not ticket:
            ...
            ticket = self.reports.create_ticket(tg_user_id=tg_user_id, tg_chat_id=tg_chat_id, tg_username=tg_username, tg_full_name=tg_full_name, status="open", topic=topic, last_message_preview=body[:200], assigned_admin_id=None)
        ...
        self.reports.create_message(ticket_id=ticket.id, direction="user", admin_id=None, tg_user_id=tg_user_id, body=body)
        self.session.commit()
        return ticket
    ...
    async def reply_from_admin(self, actor: Admin, ticket_id: int, body: str):
        ...
        self.reports.create_message(ticket_id=ticket.id, direction="admin", admin_id=actor.id, tg_user_id=ticket.tg_user_id, body=body)
