...
class MediaUploadService:
    async def upload_uploaded_file(
        ...
        if not settings.telegram_bot_token:
            raise ValidationError("Не настроен TELEGRAM_BOT_TOKEN.")

        chat_id = settings.resolved_media_upload_chat_id
        if not chat_id:
            raise ValidationError(
                "Загрузка файлом недоступна: не настроен TELEGRAM_MEDIA_UPLOAD_CHAT_ID. "
                "Либо укажи external_url, либо добавь TELEGRAM_MEDIA_UPLOAD_CHAT_ID в ENV."
            )
