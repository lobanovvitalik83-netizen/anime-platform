...
async def fallback_text_handler(message: Message) -> None:
    ...
    if mode == USER_MODE_LOOKUP and not text.isdigit():
        await message.answer(
            "Для поиска нужен только цифровой код. Нажми «Репорт», если хочешь написать в поддержку.",
            reply_markup=build_main_menu(),
        )
        return

    if mode == USER_MODE_REPORT:
        # report mode is handled by report_support.py for ordinary text
        return

    clear_user_mode(message.from_user.id)
    await message.answer(
        "Выбери действие кнопкой ниже: поиск по коду, репорт или помощь.",
        reply_markup=build_main_menu(),
    )
