    ...
    if current_admin:
        ...
        if db is not None:
            try:
                ...
                unread_notifications = 0
    return templates.TemplateResponse(name, {"request": request, "current_permissions": current_permissions, "permission_labels": PERMISSION_LABELS, "unread_notifications": unread_notifications, **context})
...
def forgot_password_page(request: Request):
    return render_template("forgot_password.html", request, db=None, page_title="Забыл пароль", current_admin=None, error=None, success=None)
...
async def forgot_password_submit(request: Request, db: Session = Depends(get_db_session)):
    ...
    if not username:
        return render_template("forgot_password.html", request, db=db, page_title="Забыл пароль", current_admin=None, error="Укажи логин.", success=None)
