...
def admin_profile_page(request: Request, db: Session = Depends(get_db_session)):
    ...
    if redirect:
        return redirect
    return render_template("profile.html", request, page_title="Мой профиль", current_admin=current_admin, current_admin_achievements=AchievementService(db).list_admin_grants(current_admin.id), error=None, success=None, password_error=None, password_success=None)
...
async def admin_profile_submit(request: Request, db: Session = Depends(get_db_session)):
    ...
    try:
        ...
        return render_template("profile.html", request, page_title="Мой профиль", current_admin=refreshed, current_admin_achievements=AchievementService(db).list_admin_grants(refreshed.id), error=None, success="Профиль обновлён.", password_error=None, password_success=None)
    except Exception as exc:
        db.rollback()
        refreshed = AuthService(db).get_admin(current_admin.id)
        return render_template("profile.html", request, page_title="Мой профиль", current_admin=refreshed, current_admin_achievements=AchievementService(db).list_admin_grants(refreshed.id), error=str(exc), success=None, password_error=None, password_success=None)
...
async def admin_profile_password_submit(request: Request, db: Session = Depends(get_db_session)):
    ...
    try:
        ...
        refreshed = AuthService(db).get_admin(current_admin.id)
        return render_template("profile.html", request, page_title="Мой профиль", current_admin=refreshed, current_admin_achievements=AchievementService(db).list_admin_grants(refreshed.id), error=None, success=None, password_error=None, password_success="Пароль изменён.")
    except Exception as exc:
        db.rollback()
        refreshed = AuthService(db).get_admin(current_admin.id)
        return render_template("profile.html", request, page_title="Мой профиль", current_admin=refreshed, current_admin_achievements=AchievementService(db).list_admin_grants(refreshed.id), error=None, success=None, password_error=str(exc), password_success=None)
...
def admin_person_profile(admin_id: int, request: Request, db: Session = Depends(get_db_session)):
    ...
    return render_template("person_profile.html", request, page_title=person.full_name or person.username, current_admin=current_admin, person=person, person_achievements=AchievementService(db).list_admin_grants(person.id), messages_enabled=_messages_enabled(db))
...
def admin_person_message(admin_id: int, request: Request, db: Session = Depends(get_db_session)):
    ...
    try:
        ...
        db.rollback()
        person = AuthService(db).get_admin(admin_id)
        return render_template("person_profile.html", request, page_title=person.full_name or person.username, current_admin=current_admin, person=person, person_achievements=AchievementService(db).list_admin_grants(person.id), messages_enabled=_messages_enabled(db), error=str(exc))
...
async def admin_team_new_submit(request: Request, db: Session = Depends(get_db_session)):
    ...
    try:
        ...
        db.rollback()
        return render_template("team_form.html", request, page_title="Создать пользователя", current_admin=current_admin, error=str(exc), success=None, credentials=None, values=values, allowed_roles=allowed_roles, action_url="/admin/team/new", submit_label="Создать пользователя", is_edit=False, reset_password_url=None, create_reset_link_url=None)
