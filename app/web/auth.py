...
def set_auth_cookie(response: RedirectResponse, admin_id: int) -> None:
    ...
    response.set_cookie(key=settings.session_cookie_name, value=token, max_age=60 * 60 * 24 * 7, secure=settings.session_cookie_secure, httponly=settings.session_cookie_httponly, samesite=settings.session_cookie_samesite, path="/")
